...
{
    [SmartPart]
    public partial class BillingBookNoneCallingBillView : UserControl, IBillingBookNoneCallingBillView
    {
        ...
        public BillingBookNoneCallingBillView()
        {
            InitializeComponent();
        ...
        /// </summary>
        [CreateNew]
        public BillingBookNoneCallingBillViewPresenter Presenter
        {
            set
            {
                _presenter = value;
        ...
        public Color RowHeaderBackColor
        {
            set
            {
                this.billBookCallingDataGV.RowHeadersDefaultCellStyle.BackColor = value;
            }
        }
        ...
        {
            //List<CallingBillInfo> caList = new List<CallingBillInfo>();
        ...
        private void billBookCallingDataGV_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                //check for removable row 
                if (billBookCallingDataGV.Rows[e.RowIndex].Cells["AllowRemove"].Value.ToString() == "1")
                {
                    CallingBillInfo billInfo = _billInfoList[e.RowIndex];
                    billBookCallingDataGV.Rows.RemoveAt(e.RowIndex);
                    _presenter.RemoveExceptionalCallingBillActivated(billInfo);
                    FillSummaryGv();
                }
                else
                {                                       
                    string message = "ไม่สามารถนำบิลหมายเลข {0} ใส่เข้าไปในสมุดจ่ายบิลได้ เนื่องจากเป็นบิลงดจ่ายไฟ";
                    message = String.Format(message, billBookCallingDataGV.Rows[e.RowIndex].Cells["InvoiceNo"].Value.ToString());
                    MessageBox.Show(null, message, "คำเตือน", MessageBoxButtons.OK, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1);
                }
            }
        }
        ...
        private void removeBt_Click(object sender, EventArgs e)
        {
            ...
            for (int i = billBookCallingDataGV.Rows.Count - 1; i >= 0; i--)
            {
                ...
                {                                        
                    ...
                    if (marked)
                    {
                        if (r.Cells["AllowRemove"].Value.ToString() == "0")
                            {
                                string message = "ไม่สามารถนำบิลหมายเลข {0} ใส่เข้าไปในสมุดจ่ายบิลได้ เนื่องจากเป็นบิลงดจ่ายไฟ";
                                message = String.Format(message, r.Cells["InvoiceNo"].Value.ToString());
                                MessageBox.Show(null, message, "คำเตือน", MessageBoxButtons.OK, MessageBoxIcon.Warning, MessageBoxDefaultButton.Button1);
                            }
                        else
                        {
