...
{
    [SmartPart]
    public partial class BillBookCallingBillView : UserControl, IBillBookCallingBillView
    {
        ...
        public BillBookCallingBillView()
        {
            InitializeComponent();
        ...
        /// </summary>
        [CreateNew]
        public BillBookCallingBillViewPresenter Presenter
        {
            set
            {
                _presenter = value;
        ...
        private void billBookCallingDataGV_CellDoubleClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            {
                CallingBillInfo billInfo = _billInfoList[e.RowIndex].Clone();
                _billInfoList.RemoveAt(e.RowIndex);
        ...
        {
            if (billBookCallingDataGV.Rows.Count == 0)
            {
        ...
        private void FindInvoice(string branchId, string mruId, string caId)
        {
            ...
            for (int i = 0; i < _billInfoList.Count; i++)
            {
                if (string.Equals(_billInfoList[i].PeaCode, branchId, StringComparison.CurrentCultureIgnoreCase) && _billInfoList[i].LineId == mruId && _billInfoList[i].CaId == caId)
                {
                    ...
                    //might have more than one CA in the list
                    //break;
                }
            }
        }
        ...
        private void caIdtext_KeyDown(object sender, KeyEventArgs e)
        {
            if (e.KeyCode == Keys.Enter)
            {
                ...
                {
                    MessageBox.Show(null, "ป้อนข้อมูลไม่ถูกต้อง", "ป้อนผิด", MessageBoxButtons.OK, MessageBoxIcon.Warning);
                }
            }
        }
