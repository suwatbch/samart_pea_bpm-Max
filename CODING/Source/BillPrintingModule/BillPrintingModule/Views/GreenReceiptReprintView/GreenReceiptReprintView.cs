...
{
    [SmartPart]
    public partial class GreenReceiptReprintView : UserControl, IGreenReceiptReprintView
    {
        ...
        public GreenReceiptReprintView()
        {
            InitializeComponent();           
        }
        ...
        /// </summary>
        [CreateNew]
        public GreenReceiptReprintViewPresenter Presenter
        {
            set
            {
                _presenter = value;
        ...
        private void Print()
        {
            if (ValidateBeforeSubmit())
            {
                    ...
                    int arraySize = receiptReprintDataGridView.Rows.Count;
                    ...
                    param.BillType = (int)BillType.ReprintGreenReceipt;
                    ...
                    if (printMruRadioButton.Checked == true)
                    {
                        ...
                        if (receiptReprintDataGridView.Rows.Count != 0)
                        {                            
                            for (int i = 0; i < receiptReprintDataGridView.Rows.Count; i++)
                            {
                                groupElectricId[i] = receiptReprintDataGridView.Rows[i].Cells["electricityIdColumn"].Value.ToString();
                    ...
                    else if (printUserRadioButton.Checked == true)
                    {
                        ...
                        if (receiptReprintDataGridView.Rows.Count != 0)
                        {
                            for (int i = 0; i < receiptReprintDataGridView.Rows.Count; i++)
                            {
                                ...
                                groupFromToElectricId[i] = receiptReprintDataGridView.Rows[i].Cells["fullIdColumn"].Value.ToString();
                    ...
                    else if (printBillSeqRadioButton.Checked == true)
                    {
                        ...
                        if (receiptReprintDataGridView.Rows.Count != 0)
                        {
                            for (int i = 0; i < receiptReprintDataGridView.Rows.Count; i++)
                            {
                                ...
                                groupFromToNumberId[i] = receiptReprintDataGridView.Rows[i].Cells["billSeqColumn"].Value.ToString();
                    ...
                    _presenter.ReprintGreenReceiptBill(param);
                ...
                if (receiptReprintDataGridView.Rows.Count != 0)
                {
                    if (rbFlag != "M")// if i don't check this line, it'll appear dialog box twice.
                    {
                        ...
                        if (dr == DialogResult.Yes)
                        {
                            ...
                            ShowGirdViewColumn();
                            receiptReprintDataGridView.Rows.Clear();
                            rbFlag = "M";
                        }
                        else
                        {
        ...
        private void dualBillReprintDataGridView_CellContentClick(object sender, DataGridViewCellEventArgs e)
        {
            ...
            else if (receiptReprintDataGridView.Columns[e.ColumnIndex].Name == "delColumn")
                receiptReprintDataGridView.Rows.RemoveAt(e.RowIndex);
        }
        ...
        private void ShowGirdViewColumn()
        {
            if (printMruRadioButton.Checked == true)
            {
                receiptReprintDataGridView.Columns["electricityIdColumn"].AutoSizeMode = DataGridViewAutoSizeColumnMode.Fill;
                receiptReprintDataGridView.Columns["electricityIdColumn"].Visible = true;
                receiptReprintDataGridView.Columns["fromNumberColumn"].Visible = false;
                receiptReprintDataGridView.Columns["toNumberColumn"].Visible = false;
            }

            if (printBillSeqRadioButton.Checked == true)
            {
                receiptReprintDataGridView.Columns["electricityIdColumn"].Visible = false;
                receiptReprintDataGridView.Columns["fromNumberColumn"].Visible = true;
                receiptReprintDataGridView.Columns["toNumberColumn"].Visible = true;
            }
        ...
        private void AddToDataGridView(string txtId, string txtFromId, string txtToId)
        {
            ...
            receiptReprintDataGridView.Enabled = true;
            ...
            if (isRepeatedItem == false)
            {
                try
                {
                    int i = receiptReprintDataGridView.Rows.Count;
                    receiptReprintDataGridView.Rows.Add(1);
                    receiptReprintDataGridView.Rows[i].Selected = true;
                    if (printMruRadioButton.Checked == true)
                    {
                        receiptReprintDataGridView.Rows[i].Cells["electricityIdColumn"].Value = txtId;
                    }
                    ...
                    if (printUserRadioButton.Checked == true)
                    {
                        ...
                        receiptReprintDataGridView.Rows[i].Cells["fromNumberColumn"].Value = txtFromId;
                        receiptReprintDataGridView.Rows[i].Cells["toNumberColumn"].Value = txtToId;

                        //insert into column "fullId", visible = false
                        receiptReprintDataGridView.Rows[i].Cells["fullIdColumn"].Value = "UserId" + dash + txtFromId + dash + txtToId;
                    }
                    ...
                    if (printBillSeqRadioButton.Checked == true)
                    {
                        ...
                        receiptReprintDataGridView.Rows[i].Cells["fromNumberColumn"].Value = txtFromId;
                        receiptReprintDataGridView.Rows[i].Cells["toNumberColumn"].Value = txtToId;
                        receiptReprintDataGridView.Rows[i].Cells["billSeqColumn"].Value = "BillSeq" + dash + txtFromId + dash + txtToId;
                    }

                    //when selected = true, bgColor = blue so we didn't need it.
                    for (int j = 0; j <= i - 1; j++)
                    {
                        receiptReprintDataGridView.Rows[j].Selected = false;
                    }
            ...
            if (printMruRadioButton.Checked == true)
            {
                if (receiptReprintDataGridView.Rows.Count != 0)
                {
                    for (int i = 0; i < receiptReprintDataGridView.Rows.Count; i++)
                    {
                        ...
                        if (txtId == (string)receiptReprintDataGridView.Rows[i].Cells["electricityIdColumn"].Value)
                        {
                            isRepeated = true;
                            break;
                        }
        ...
        private void InitializeControlValue()
        {
            ...
            receiptReprintDataGridView.Rows.Clear();
            receiptReprintDataGridView.Enabled = false;
            receiptReprintDataGridView.AutoGenerateColumns = false;
        ...
        private bool ValidateBeforeSubmit()
        {
            ...
            if (printBillSeqRadioButton.Checked == false)
            {
                ...
                else if (receiptReprintDataGridView.Rows.Count == 0)
                {
                    isValidated = false;
