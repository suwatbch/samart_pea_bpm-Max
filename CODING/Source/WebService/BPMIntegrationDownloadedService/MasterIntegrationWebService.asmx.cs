...
namespace PEA.BPM.WebService.BPMIntegrationDownloadedService
{
    ...
    public class MasterIntegrationWebService : BaseWebService
    {
        ...
        [SoapHeader("AuthInfoValue"), WebMethod]
        public CompressData GetUpdateOwnBank(string branchId, DateTime lastModifiedDate, DateTime serverDate)
        {
            WebServiceSecurity.CheckAuthorization(this.AuthInfoValue);

            return ServiceHelper.CompressData<List<OwnBankInfo>>(_bs.GetUpdateOwnBank(branchId, lastModifiedDate, serverDate));
        }
