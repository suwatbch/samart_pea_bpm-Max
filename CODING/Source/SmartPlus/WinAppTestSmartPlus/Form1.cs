...
namespace WinAppTestSmartPlus
{
    public partial class Form1 : Form
    {
        ...
        private void btnSearchContractorService_Click(object sender, EventArgs e)
        {
            ...
            gvArResult.Rows.Clear();
            gvArResult.Refresh();
            try
            {
                string  caid    = txtCaId.Text.Trim();
                string  flag    = cboBillFlag.Text;
                var     result  = WS.SearchContractorServiceWithBillFlag(caid, flag);
                int     RecCnt  = 0;
        ...
        private void txtCaId_TextChanged(object sender, EventArgs e)
        {
            txtInterestCaId.Text = txtCaId.Text;
        }

        private void btnCallInterestByDate_Click(object sender, EventArgs e)
        {
            btnCallInterestByDate.Enabled = false;
            ...
            try
            {
                ...
                string tmpDt          = txtInterestDay.Text.Trim();
                DateTime InterestDate = DateTime.Now;
                try
                {
                    InterestDate = Convert.ToDateTime(tmpDt);
                }
                catch (Exception)
                {                    
                    MessageBox.Show("รูปแบบวันที่ คือ YYYY-mm-dd เช่น 2021-06-10");
                    btnCallInterestByDate.Enabled = true;
                    throw;
                }
                
                var result = WS.SearchContractorServiceWithInterestDate(caid, InterestDate);
                int RecCnt = 0;
                foreach (var r in result)
                {
                    string invoiceNo = null;
