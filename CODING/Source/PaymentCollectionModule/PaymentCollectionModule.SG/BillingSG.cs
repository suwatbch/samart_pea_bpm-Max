...
using PaymentCollectionModule.SG.BillingWCF;
...
namespace PaymentCollectionModule.SG
{
    public class BillingSG : IBillingService
    {
        ...
        public string CheckSettingGroupReceiptEnableStatus()
        {
            try
            {
                return _ws.CheckSettingGroupReceiptEnableStatus();
            }
            catch (Exception ex)
            {
                _ws.Abort();
        ...
        {
            QRPaymentResponse result = null; 
            try
            {
                string qrPaymentCheckStatusUrl = CodeTable.Instant.GetAppSettingValue("QRPayment_Check_Status_URL");
                string qrPaymentAuthenKey = CodeTable.Instant.GetAppSettingValue("QRPayment_KEY");

                //http post to pea service check status qr payment. 
                WebClientEx client = new WebClientEx();

                client.Headers.Add("Content-Type", "application/json");
                client.Headers.Add("accept", "*/*");

                string bodyString = string.Format("{{\"QrTransactionId\": \"\",\"TRSG\": \"{0}\",\"Ref1\": \"{1}\",\"Ref2\": \"{2}\",\"Ref3\": \"\",\"Remark\": \"\",\"ApiKey\": \"{3}\"}}",
                    Session.Branch.Id, qrpaymentInfo.ref1, qrpaymentInfo.ref2, qrPaymentAuthenKey);
                ...
                string jsonString = Encoding.UTF8.GetString(client.UploadData(qrPaymentCheckStatusUrl, "POST",
                                    Encoding.UTF8.GetBytes(bodyString)));

                using (var ms = new MemoryStream(Encoding.Unicode.GetBytes(jsonString)))
                {
                    var serializer = new DataContractJsonSerializer(typeof(QRPaymentResponse));
                    var objectdata = (QRPaymentResponse)serializer.ReadObject(ms);
            ...
            catch (Exception ex)
            {
                ...
                result = new QRPaymentResponse();
                result.Status = false;

                if (ex.Message != null && ( ex.Message == "The operation has timed out" || ex.Message == "Unable to connect to the remote server"))
                    result.ErrorMessage = "ไม่สามารถติดต่อ Service สอบถามสถานะ QR Payment";
            }
        ...
        {
            QRRefundResponse result = null;
            try
            {
                string qrPaymentCheckStatusUrl = CodeTable.Instant.GetAppSettingValue("QRPayment_Refund_URL");
                string qrPaymentAuthenKey = CodeTable.Instant.GetAppSettingValue("QRPayment_KEY");

                //http post to pea service check status qr payment. 
                WebClientEx client = new WebClientEx();
                client.Headers.Add("Content-Type", "application/json");
                client.Headers.Add("Authorization", qrPaymentAuthenKey);
                client.Headers.Add("accept", "*/*");
                ...
                try
                {
                    using (var ms = new MemoryStream(Encoding.Unicode.GetBytes(jsonString)))
                    {
                        var serializer = new DataContractJsonSerializer(typeof(QRRefundResponse));
                        var objectdata = (QRRefundResponse)serializer.ReadObject(ms);
        ...
        public string QRPaymentMethodByBranchId(string branchId)
        {
            try
            {
                return _ws.QRPaymentMethodByBranchId(branchId); 
            }
            catch (Exception ex)
            {
                _ws.Abort();
        ...
        public QRRefundResponse QRPostOfflinePayment(QRPaymentInfo qrpaymentInfo)
        {
            ...
            try
            {
                string qrPaymentCheckStatusUrl = CodeTable.Instant.GetAppSettingValue("QRPayment_PostOfflinePay_URL");
                string qrPaymentAuthenKey = CodeTable.Instant.GetAppSettingValue("QRPayment_KEY");
                ...
                string bodyString = string.Format("{{\"QrTransactionId\": \"\",\"TRSG\": \"{0}\",\"Ref1\": \"{1}\",\"Ref2\": \"{2}\",\"Ref3\": \"\",\"PayBankRef\": \"{3}\",\"Remark\": \"\",\"ApiKey\": \"{4}\"}}",
                    Session.Branch.Id, qrpaymentInfo.ref1, qrpaymentInfo.ref2,qrpaymentInfo.referenceNo, qrPaymentAuthenKey);
