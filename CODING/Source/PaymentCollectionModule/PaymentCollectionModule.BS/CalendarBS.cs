...
namespace PEA.BPM.PaymentCollectionModule.BS
{
    public  class CalendarBS
    {
        ...
        string _branchId;
        public CalendarBS(string branchId)
        {
            CalendarDA calendar = new CalendarDA();
            this._branchId = branchId;
            _calendar = calendar.GetCalendar(branchId);
            initHoliday();
        }
        ...
        {
            foreach (DataRow  row in _calendar.Rows)
            {
                DateTime nwDate = (DateTime)DaHelper.GetDate(row, "NWDay");
                CultureInfo th_culture = new CultureInfo("th-TH");
                _th_dt = th_culture.DateTimeFormat;
                string tempDate = nwDate.ToString("yyyyMMdd", _th_dt);
                _holidays.Add(tempDate, tempDate);
            }
        }
