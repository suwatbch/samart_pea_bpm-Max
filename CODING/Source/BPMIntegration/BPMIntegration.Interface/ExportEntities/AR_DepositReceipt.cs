...
{
    public class AR_DepositReceipt : IListUtility<AR_DepositReceipt>
    {
        string _action;
        string _posRSG;
        string _ptId;
        ...
        #region IListUtility<AR_DepositReceipt> Members
        ...
        public string ToStream()
        {
            ...
            string[] template = { Action == null ? string.Empty : Action, 
                                    ...
                                    string.Empty, //MainSub == null ? string.Empty : MainSub, -- SAP doesn't need main/sub, can identify by filetype
                                    Name == null ? string.Empty : Name, 
                                    Address == null ? string.Empty : Address, 
                                    Description == null ? string.Empty : Description,
                                    Qty == null ? string.Empty : Qty.ToString(), 
                                    PriceUnit == null ? string.Empty : PriceUnit.Value.ToString("#0.00"), 
                                    BaseAmount == null ? string.Empty : BaseAmount.Value.ToString("#0.00"), 
                                    VatCode == null ? string.Empty : VatCode, 
                                    VatAmount == null ? string.Empty : VatAmount.Value.ToString("#0.00"), 
                                    DisconnectDoc == null ? string.Empty : DisconnectDoc, 
                                    OriginalInvoiceNo == null ? string.Empty : OriginalInvoiceNo,
                                    PaymentDt  == null ? string.Empty : PaymentDt.Value.ToString("HH:mm:ss", formatDate) };            
            return string.Join("|", template);
        }

        public AR_DepositReceipt ParseExtract(string txt)
        {
