...
{
    public class DL200_UPLOAD_EXPORT_TABLE_JOB : IJob
    {
        private const string DL_NAME_ID = "DL200";
        private const string DL_FULL_NAME = "DL200_UPLOAD_EXPORT_TABLE_JOB";
        ...
        public DL200_UPLOAD_EXPORT_TABLE_JOB()
        {
            ...
            try
            {
                switch (tb)
                {
                    case "ExportTransactionLog":
                        {
                            param.FileName = "ExportTransactionLog";
                            ret = UploadExportTable(param);
                        }
                        break;                 
                    default:
                        {
        ...
        private bool UploadExportTable(ACABatchParam param)
        {
            _entityName = typeof(ExportTransactionLogInfo).Name;
            //_serverDate = _bpmServerSG.GetServerTime();
            string _infName = "ExportTable";

            try
            {
                List<ExportTransactionLogInfo> list = new List<ExportTransactionLogInfo>();
                list = _bpmBranchService.GetToUploadExportTransactionLog(_serverDate);
                ...
                if (listCount > 0)
                {
                    ...
                    rows = _bpmServerSG.UploadExportTransactionLog(list, _branchId);
            ...
            {
                int syncRows = _bpmBranchService.SetSyncExportTransactionLog(_serverDate);
