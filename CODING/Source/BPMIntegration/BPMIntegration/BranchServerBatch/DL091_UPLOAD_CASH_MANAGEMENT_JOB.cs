...
{
    public class DL091_UPLOAD_CASH_MANAGEMENT_JOB : IJob
    {
        private const string DL_NAME_ID = "DL091";
        private const string DL_FULL_NAME = "DL091_UPLOAD_CASH_MANAGEMENT";
        ...
        public DL091_UPLOAD_CASH_MANAGEMENT_JOB()
        {
                    ...
                    case "CashierWorkStatus":
                        {
                            ...
                            ret = UploadCashierWorkStatus(param);
                            ret = true;
                        }
                        break;
                    case "CashierMoneyTransfer":
                        {
                            ...
                            ret = UploadCashierMoneyTransfer(param);
                            ret = true;
                        }
                        break;
                    case "CashierMoneyFlow":
                        {
                            ...
                            ret = UploadCashierMoneyFlow(param);
                            ret = true;
                        }
                        break;
                    case "CashierMoneyFlowItem":
                        {
                            ...
                            ret = UploadCashierMoneyFlowItem(param);
                            ret = true;
                        }
                        break;
                    default:
        ...
        private bool UploadCashierWorkStatus(ACABatchParam param)
        {
            _entityName = typeof(CashierWorkStatusInfo).Name;
            //_serverDate = _bpmServerSG.GetServerTime();
            string _infName = "CashierWorkStatus";

            try
            {
                List<CashierWorkStatusInfo> list = new List<CashierWorkStatusInfo>();
                list = _bpmBranchService.GetToUploadCashierWorkStatus(_serverDate);
                ...
                if (listCount > 0)
                {
                    ...
                    rows = _bpmServerSG.UploadCashierWorkStatus(list, _branchId);
            ...
            {
                int syncRows = _bpmBranchService.SetSyncCashierWorkStatus(_serverDate);
                return _logger.HandleLog(param.BatchKey, "3", _infName, _entityName, _localDate, _serverDate, syncRows, ex.ToString());
            }

        }

        private bool UploadCashierMoneyTransfer(ACABatchParam param)
        {
            _entityName = typeof(CashierMoneyTransferInfo).Name;
            //_serverDate = _bpmServerSG.GetServerTime();
            string _infName = "CashierMoneyTransfer";

            try
            {
                List<CashierMoneyTransferInfo> list = new List<CashierMoneyTransferInfo>();
                list = _bpmBranchService.GetToUploadCashierMoneyTransfer(_serverDate);
                int listCount = list.Count;
                int rows = 0;
                ...
                if (listCount > 0)
                {
                    ...
                    rows = _bpmServerSG.UploadCashierMoneyTransfer(list, _branchId);
            ...
            {
                int syncRows = _bpmBranchService.SetSyncCashierMoneyTransfer(_serverDate);
                return _logger.HandleLog(param.BatchKey, "3", _infName, _entityName, _localDate, _serverDate, syncRows, ex.ToString());
            }

        }

        private bool UploadCashierMoneyFlow(ACABatchParam param)
        {
            _entityName = typeof(CashierMoneyFlowInfo).Name;
            //_serverDate = _bpmServerSG.GetServerTime();
            string _infName = "CashierMoneyFlow";

            try
            {
                List<CashierMoneyFlowInfo> list = new List<CashierMoneyFlowInfo>();
                list = _bpmBranchService.GetToUploadCashierMoneyFlow(_serverDate);
                int listCount = list.Count;
                int rows = 0;
                ...
                if (listCount > 0)
                {
                    ...
                    rows = _bpmServerSG.UploadCashierMoneyFlow(list, _branchId);
            ...
            {
                int syncRows = _bpmBranchService.SetSyncCashierMoneyFlow(_serverDate);
                return _logger.HandleLog(param.BatchKey, "3", _infName, _entityName, _localDate, _serverDate, syncRows, ex.ToString());
            }
        }

        private bool UploadCashierMoneyFlowItem(ACABatchParam param)
        {
            _entityName = typeof(CashierMoneyFlowItemInfo).Name;
            //_serverDate = _bpmServerSG.GetServerTime();
            string _infName = "CashierMoneyFlowItem";

            try
            {
                List<CashierMoneyFlowItemInfo> list = new List<CashierMoneyFlowItemInfo>();
                list = _bpmBranchService.GetToUploadCashierMoneyFlowItem(_serverDate);
                int listCount = list.Count;
                int rows = 0;
                ...
                if (listCount > 0)
                {
                    ...
                    rows = _bpmServerSG.UploadCashierMoneyFlowItem(list, _branchId);
            ...
            {
                int syncRows = _bpmBranchService.SetSyncCashierMoneyFlowItem(_serverDate);
                return _logger.HandleLog(param.BatchKey, "3", _infName, _entityName, _localDate, _serverDate, syncRows, ex.ToString());
            }
        }
