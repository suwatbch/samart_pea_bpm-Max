...
{
    public class DL071_UPLOAD_MASTER_JOB : IJob
    {
        private const string DL_NAME_ID = "DL071";
        private const string DL_FULL_NAME = "DL071_UPLOAD_MASTER";
        ...
        public DL071_UPLOAD_MASTER_JOB()
        {
        ...
        private bool UploadMRUPlan(ACABatchParam param)
        {
            _entityName = typeof(MRUPlanInfo).Name;                     
            //_serverDate = _bpmServerSG.GetServerTime();
            string _infName = "MRUPlan";

            try
            {
                List<MRUPlanInfo> list = new List<MRUPlanInfo>();
                list = _bpmBranchService.GetToUploadMRUPlan(_serverDate);
                ...
                if (listCount > 0)
                {
                    ...
                    rows = _bpmServerSG.UploadMRUPlan(list, _branchId);
            ...
            {
                int syncRows = _bpmBranchService.SetSyncMRUPlan(_serverDate);
