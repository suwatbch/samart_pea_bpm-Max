...
{
    public class DL101_UPLOAD_EPAYMENT_JOB : IJob
    {
        private const string DL_NAME_ID = "DL101";
        private const string DL_FULL_NAME = "DL101_UPLOAD_EPAYMENT";
        ...
        public DL101_UPLOAD_EPAYMENT_JOB()
        {
        ...
        private bool ExportData(Guid batchKey, string tb, bool overwrite)
        {
            ...
            try
            {
                switch (tb)
                {
                    case "EPayClarify":
                        {
                            param.FileName = "EPayClarify";
                            ret = UploadEPayClearify(param);
                        }
                        break;                 
                    default:
                        {
        ...
        private bool UploadEPayClearify(ACABatchParam param)
        {
            _entityName = typeof(EPayClarifyInfo).Name;
            //_serverDate = _bpmServerSG.GetServerTime();
            string _infName = "EPayClearify";

            try
            {
                List<EPayClarifyInfo> list = new List<EPayClarifyInfo>();
                list = _bpmBranchService.GetToUploadEPayClarify(_serverDate);
                ...
                if (listCount > 0)
                {
                    ...
                    rows = _bpmServerSG.UploadEPayClarify(list, _branchId);
            ...
            {
                int syncRows = _bpmBranchService.SetSyncEPayClarify(_serverDate);
