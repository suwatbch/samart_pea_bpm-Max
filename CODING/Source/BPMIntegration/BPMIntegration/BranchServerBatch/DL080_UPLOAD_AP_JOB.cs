...
{
    public class DL080_UPLOAD_AP_JOB : IJob
    {
        private const string DL_NAME_ID = "DL080";
        private const string DL_FULL_NAME = "DL080_UPLOAD_AP";
                    ...
                    case "APChequePayment":
                        {
                            ...
                            ret = UploadAPChequePayment(param);
                        }
                        break;
                    default:
                        {
        ...
        private bool UploadAP(ACABatchParam param)
        {
            ...
            try
            {
                ...
                list = _bpmBranchService.GetToUploadAP(_serverDate);
                ...
                if (listCount > 0)
                {
                    ...
                    rows = _bpmServerSG.UploadAP(list, _branchId);
            ...
            {
                int syncRows = _bpmBranchService.SetSyncAP(_serverDate);
                return _logger.HandleLog(param.BatchKey, "3", _infName, _entityName, _localDate, _serverDate, syncRows);
            }
        }

        private bool UploadAPChequePayment(ACABatchParam param)
        {
            _entityName = typeof(APChequePaymentInfo).Name;
            //_serverDate = _bpmServerSG.GetServerTime();
            string _infName = "APChequePayment";

            try
            {
                List<APChequePaymentInfo> list = new List<APChequePaymentInfo>();
                list = _bpmBranchService.GetToUploadAPChequePayment(_serverDate);
                int listCount = list.Count;
                int rows = 0;
                ...
                if (listCount > 0)
                {
                    ...
                    rows = _bpmServerSG.UploadAPChequePayment(list, _branchId);
            ...
            {
                int syncRows = _bpmBranchService.SetSyncAPChequePayment(_serverDate);
